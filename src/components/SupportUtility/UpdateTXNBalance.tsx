// src/components/SupportUtility/UpdateTxnBalanceModal.tsx
import { useMemo, useState } from "react";
import type { ReactNode } from "react";
import { User, Building2, ChevronDown, MoreVertical, X, Check, AlertCircle, Search } from "lucide-react";
import Image from "@/components/ui/Image";
import FormModal from "@/components/shared/FormModal";

export interface UpdateTxnBalanceModalProps {
  open: boolean;
  onClose: () => void;
}
...
const SelectFieldWithMenu = ({ icon, value, onMenuClick, menuActive, error }: any) => (
...
const SuccessModal = ({ title = "Update Successfully", subtitle = "TXN Balance has been Updated Successfully", onClose }: any) => (
...
export default function UpdateTxnBalanceModal({ open, onClose }: UpdateTxnBalanceModalProps) {
  ...
  return (
    <>
      <FormModal
        ...
        headerIcon={<div className="flex h-15 w-12 items-center justify-center"><Image src="/User.png" alt="Update TXN Balance" width={50} height={50} /></div>}
        tabs={[]}
        activeTab=""
        onTabChange={() => {}}
        hideFooter
...
/>
              ...
              <FieldWrap label="Branch Name" labelHi="शाखेचे नाव">
                <TextField icon={<Building2 size={16} />} value={branchName} placeholder="name@company.com" disabled />
              </FieldWrap>
            </div>
          </div>
        </div>
      ...
      {isSuccessOpen && <SuccessModal title="Update Successfully" subtitle="TXN Balance has been Updated Successfully" onClose={handleSuccessClose} />}
    </>
  );
}