...
import { X, Filter as FilterIcon, Landmark, Hash } from "lucide-react";
...
export type ReconciliationFilters = Record<FilterKey, string>;

type ReconciliationFilterModalProps = {
  onClose: () => void;
  onApply: (filters: ReconciliationFilters) => void;
  initialValues?: ReconciliationFilters;
};
...
export default function ReconciliationFilterModal({
  onClose,
  onApply,
  initialValues = defaultReconciliationFilters,
}: ReconciliationFilterModalProps) {
  const [activeFilter, setActiveFilter] = useState<FilterKey>("branchName");
  const [values, setValues] = useState<ReconciliationFilters>(initialValues);
  ...
  const handleClearAll = () => {
    setValues(defaultReconciliationFilters);
