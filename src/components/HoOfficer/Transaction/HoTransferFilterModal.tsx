...
export type HoTransferFilters = Record<FilterKey, string>;

type HoTransferFilterModalProps = {
  onClose: () => void;
  onApply: (filters: HoTransferFilters) => void;
  initialValues?: HoTransferFilters;
};

export const defaultHoTransferFilters: HoTransferFilters = {
  accountName: "",
  accountCode: "",
  scrollNo: "",
};

export default function HoTransferFilterModal({
  onClose,
  onApply,
  initialValues = defaultHoTransferFilters,
}: HoTransferFilterModalProps) {
  const [activeFilter, setActiveFilter] = useState<FilterKey>("accountName");
  const [values, setValues] = useState<HoTransferFilters>(initialValues);
  ...
  const handleClearAll = () => {
    setValues(defaultHoTransferFilters);
    onApply(defaultHoTransferFilters);
    onClose();
  };
