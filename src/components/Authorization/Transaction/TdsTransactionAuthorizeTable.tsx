...
import type { TdsTransactionFilters } from "./TdsTransactionFilterModal";

export type TdsTransactionAuthorizeTab = "new" | "rejected";

export type TdsTransactionAuthorizeRow = {
  srNo: number;
  scrollNo: string;
  accountCode: string;
  amount: string;
  ...
  createdBy: string;
  createdDate: string;
  tab: TdsTransactionAuthorizeTab;
};
...
const SAMPLE_TDS: Omit<TdsTransactionAuthorizeRow, "srNo" | "tab">[] = [
...
const buildRows = (tab: TdsTransactionAuthorizeTab, count: number): TdsTransactionAuthorizeRow[] =>
  Array.from({ length: count }, (_, i) => ({
    ...SAMPLE_TDS[i % SAMPLE_TDS.length],
    scrollNo: String(118 + i + (tab === "rejected" ? 100 : 0)),
    srNo: i + 1,
    tab,
  }));

export const TDS_TRANSACTION_TAB_COUNTS: Record<TdsTransactionAuthorizeTab, number> = {
  new: 20,
  rejected: 18,
};

const ALL_ROWS: TdsTransactionAuthorizeRow[] = [
  ...buildRows("new", TDS_TRANSACTION_TAB_COUNTS.new),
  ...buildRows("rejected", TDS_TRANSACTION_TAB_COUNTS.rejected),
];
...
type TdsTransactionAuthorizeTableProps = {
  activeTab: TdsTransactionAuthorizeTab;
  filters?: TdsTransactionFilters;
  onAuthorize?: (row: TdsTransactionAuthorizeRow) => void;
};

const TdsTransactionAuthorizeTable = ({ activeTab, filters, onAuthorize }: TdsTransactionAuthorizeTableProps) => {
...
export default TdsTransactionAuthorizeTable;
