...
export type RtgsFilters = Record<FilterKey, string>;

type RtgsFilterModalProps = {
  onClose: () => void;
  onApply: (filters: RtgsFilters) => void;
  initialValues?: RtgsFilters;
};

export const defaultRtgsFilters: RtgsFilters = {
  accountName: "",
  accountCode: "",
  scrollNo: "",
  amount: "",
...
export default function RtgsFilterModal({
  onClose,
  onApply,
  initialValues = defaultRtgsFilters,
}: RtgsFilterModalProps) {
  const [activeFilter, setActiveFilter] = useState<FilterKey>("accountName");
  const [values, setValues] = useState<RtgsFilters>(initialValues);
  ...
  const handleClearAll = () => {
    setValues(defaultRtgsFilters);
    onApply(defaultRtgsFilters);
    onClose();
  };
