...
export type TransferFilters = Record<FilterKey, string>;

type TransferFilterModalProps = {
  onClose: () => void;
  onApply: (filters: TransferFilters) => void;
  initialValues?: TransferFilters;
};
...
export default function TransferFilterModal({
  onClose,
  onApply,
  initialValues = defaultTransferFilters,
}: TransferFilterModalProps) {
  const [activeFilter, setActiveFilter] = useState<FilterKey>("accountName");
  const [values, setValues] = useState<TransferFilters>(initialValues);
  ...
  const handleClearAll = () => {
    setValues(defaultTransferFilters);
    onApply(defaultTransferFilters);
    onClose();
  };
