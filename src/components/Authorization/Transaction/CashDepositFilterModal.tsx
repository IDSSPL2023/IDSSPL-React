...
export type CashDepositFilters = Record<FilterKey, string>;

type CashDepositFilterModalProps = {
  onClose: () => void;
  onApply: (filters: CashDepositFilters) => void;
  initialValues?: CashDepositFilters;
};

export const defaultCashDepositFilters: CashDepositFilters = {
  accountName: "",
  accountCode: "",
  scrollNo: "",
};

export default function CashDepositFilterModal({
  onClose,
  onApply,
  initialValues = defaultCashDepositFilters,
}: CashDepositFilterModalProps) {
  const [activeFilter, setActiveFilter] = useState<FilterKey>("accountName");
  const [values, setValues] = useState<CashDepositFilters>(initialValues);
  ...
  const handleClearAll = () => {
    setValues(defaultCashDepositFilters);
    onApply(defaultCashDepositFilters);
    onClose();
  };
