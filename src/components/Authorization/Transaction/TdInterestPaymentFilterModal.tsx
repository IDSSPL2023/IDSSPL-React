...
export type TdInterestPaymentFilters = Record<FilterKey, string>;

type TdInterestPaymentFilterModalProps = {
  onClose: () => void;
  onApply: (filters: TdInterestPaymentFilters) => void;
  initialValues?: TdInterestPaymentFilters;
};

export const defaultTdInterestPaymentFilters: TdInterestPaymentFilters = {
  accountName: "",
  accountCode: "",
  scrollNo: "",
};
...
function TdInterestPaymentFilterModal({
...
}: TdInterestPaymentFilterModalProps) {
  const [activeFilter, setActiveFilter] = useState<FilterKey>("accountName");
  const [values, setValues] = useState<TdInterestPaymentFilters>(initialValues);
  ...
  const handleClearAll = () => {
    setValues(defaultTdInterestPaymentFilters);
    onApply(defaultTdInterestPaymentFilters);
    onClose();
  };
