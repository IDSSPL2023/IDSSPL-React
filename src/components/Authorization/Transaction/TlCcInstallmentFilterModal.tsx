...
import { X, Filter as FilterIcon, User, Hash, IndianRupee } from "lucide-react";
...
export type TlCcInstallmentFilters = Record<FilterKey, string>;

type TlCcInstallmentFilterModalProps = {
  onClose: () => void;
  onApply: (filters: TlCcInstallmentFilters) => void;
  initialValues?: TlCcInstallmentFilters;
};

export const defaultTlCcInstallmentFilters: TlCcInstallmentFilters = {
  accountName: "",
  accountCode: "",
  scrollNo: "",
  amount: "",
...
export default function TlCcInstallmentFilterModal({
  onClose,
  onApply,
  initialValues = defaultTlCcInstallmentFilters,
}: TlCcInstallmentFilterModalProps) {
  const [activeFilter, setActiveFilter] = useState<FilterKey>("accountName");
  const [values, setValues] = useState<TlCcInstallmentFilters>(initialValues);
  ...
  const handleClearAll = () => {
    setValues(defaultTlCcInstallmentFilters);
    onApply(defaultTlCcInstallmentFilters);
    onClose();
  };
