...
export interface AuthorizePigmyOpenData {
  customerId: string;
  customerName: string;
  categoryCode: string;
  riskCategory: string;
...
const DEFAULT_DATA: AuthorizePigmyOpenData = {
  customerId: "00012",
...
export interface AuthorizePigmyOpenProps {
  open: boolean;
  initialData?: Partial<AuthorizePigmyOpenData>;
  onClose?: () => void;
  onAuthorize?: () => void;
...
function AuthorizePigmyOpenModal({
  ...
  onAuthorize,
  onReject,
}: AuthorizePigmyOpenProps) {
  const [data, setData] = useState<AuthorizePigmyOpenData>({
    ...DEFAULT_DATA,
    ...initialData,
  });
...
export default AuthorizePigmyOpenModal;
