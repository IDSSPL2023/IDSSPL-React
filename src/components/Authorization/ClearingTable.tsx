...
const columns = [
  {
    key: "srNo",
    labelKey: "authorization.customerAuthorization.table.srNo",
    sortable: false,
    width: "80px",
  },
  {
    key: "action",
    labelKey: "authorization.customerAuthorization.table.action",
    sortable: false,
    width: "90px",
  },
  {
    key: "customerDetails",
    labelKey: "authorization.customerAuthorization.table.customerDetails",
    sortable: false,
    width: "260px",
  },
  {
    key: "status",
    labelKey: "authorization.customerAuthorization.table.status",
    sortable: true,
    width: "190px",
  },
  {
    key: "name",
    labelKey: "authorization.customerAuthorization.table.name",
    sortable: true,
    width: "200px",
  },
  {
    key: "gender",
    labelKey: "authorization.customerAuthorization.table.gender",
    sortable: true,
    width: "90px",
  },
  {
    key: "dob",
    labelKey: "authorization.customerAuthorization.table.dob",
    sortable: true,
    width: "150px",
  },
  {
    key: "regDate",
    labelKey: "authorization.customerAuthorization.table.regDate",
    sortable: true,
    width: "160px",
  },
  {
    key: "categoryCode",
    labelKey: "authorization.customerAuthorization.table.categoryCode",
    sortable: true,
    width: "140px",
  },
  {
    key: "riskCategory",
    labelKey: "authorization.customerAuthorization.table.riskCategory",
    sortable: true,
    width: "170px",
  },
] as const;
...
const SAMPLE_CUSTOMERS: Omit<RowData, "srNo" | "tab" | "status">[] = [
  {
    ...
    phone: "8989567890",
    email: "shivappa@gmail.com",
    name: "Jali Shivappa Telgi",
    gender: "M",
    dob: "18-Aug-2001",
    regDate: "25-Sep-2026",
    categoryCode: "Public",
...
const ClearingAuthorizationTable = ({
  ...
  const filteredRows = rows.filter((r) => {
    ...
    if (
      ...
      !r.name.toLowerCase().includes(filters.customerName.toLowerCase())
    )
      return false;
    if (
      filters.customerId &&
      !r.customerId.toLowerCase().includes(filters.customerId.toLowerCase())
    )
      return false;
    return true;
  });
          ...
          <tbody>
            {sortedRows.map((row, idx) => (
              ...
              >
                ...
                <td
                  className="px-6 py-3 text-[16px] text-gray-700"
                  style={{ width: "260px" }}
                >
                ...
                <td className="px-6 py-3" style={{ width: "190px" }}>
                  <StatusPill
                    label={row.status}
                    tone={TAB_STATUS_TONE[row.tab]}
                  />
                </td>

                <td
                  className="px-6 py-3 text-[16px] text-gray-700 truncate"
                  style={{ width: "200px" }}
                >
                  {row.name}
                </td>
                ...
                >
                  <span
                    className={`font-semibold ${riskColor(row.riskCategory)}`}
                  >
                    {row.riskCategory}
                  </span>
                </td>
