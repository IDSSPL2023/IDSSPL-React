...
const BRANCHES = ["Main Branch", "City Branch", "Rural Branch"];
const EMPLOYMENT_TYPES = ["Permanent", "Contract", "Temporary", "Probation"];
const EMP_STATUS = ["Active", "Inactive"];
const CAST_OPTIONS = ["Open", "OBC", "SC", "ST", "NT"];
const MARITAL_STATUS = ["Married", "UnMarried", "Divorced", "Widowed"];
...
type DocRow = { name: string; required: boolean; status: "Pending" | "Uploaded"; fileName?: string };

export type NewEmployeeData = PayrollRow;
...
const AddEmployee = ({ onClose, onSave, nextEmployeeId = "" }: AddEmployeeProps) => {
  ...
  const [documents, setDocuments] = useState<DocRow[]>(
    DOCUMENT_LIST.map((name) => ({ name, required: true, status: "Pending" }))
  );
  ...
  const VALIDATORS: Record<TabKey, () => boolean> = {
...
export { TABS as EMPLOYEE_FORM_TABS };
