...
import { X, Filter as FilterIcon, Hash, Calendar } from "lucide-react";
...
const filterOptions = [
  ...
  {
    id: "daysOfMonth",
    label: "Days of Months",
    icon: <Calendar size={18} className="text-primary" />,
    placeholder: "Days of Months",
  },
  {
    id: "workingDays",
    label: "Working Days",
...
export type AttendanceFilters = Record<FilterKey, string>;

export const defaultValues: AttendanceFilters = {
  employeeId: "",
  employeeName: "",
  daysOfMonth: "",
  workingDays: "",
...
type UpdateAttendanceFilterModalProps = {
  onClose: () => void;
  onApply: (filters: AttendanceFilters) => void;
  initialValues?: AttendanceFilters;
};

export default function UpdateAttendanceFilterModal({
  onClose,
  onApply,
  initialValues = defaultValues,
}: UpdateAttendanceFilterModalProps) {
  const [activeFilter, setActiveFilter] = useState<FilterKey>("employeeId");
  const [values, setValues] = useState<AttendanceFilters>(initialValues);
