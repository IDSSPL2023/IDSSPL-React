...
import { X, Filter as FilterIcon, Hash, Briefcase, IdCard, CalendarDays } from "lucide-react";
...
export type LeaveBalanceFilters = Record<FilterKey, string>;

export const defaultValues: LeaveBalanceFilters = {
  employeeId: "",
  employeeName: "",
  employmentType: "",
  designation: "",
...
type EmployeeLeaveBalanceFilterModalProps = {
  onClose: () => void;
  onApply: (filters: LeaveBalanceFilters) => void;
  initialValues?: LeaveBalanceFilters;
};

export default function EmployeeLeaveBalanceFilterModal({
  onClose,
  onApply,
  initialValues = defaultValues,
}: EmployeeLeaveBalanceFilterModalProps) {
  const [activeFilter, setActiveFilter] = useState<FilterKey>("employeeId");
  const [values, setValues] = useState<LeaveBalanceFilters>(initialValues);

  const active = filterOptions.find((f) => f.id === activeFilter);
  const isNumericField = activeFilter === "earnLeave" || activeFilter === "sickLeave" || activeFilter === "casualLeave";
  const isDateField = activeFilter === "date";
