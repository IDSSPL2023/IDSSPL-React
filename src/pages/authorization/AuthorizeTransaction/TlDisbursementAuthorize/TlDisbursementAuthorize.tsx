...
import { ShieldCheck, Hash, IndianRupee, X, Filter as FilterIcon, User, CreditCard, FileText, MoreVertical, ThumbsUp, ThumbsDown } from "lucide-react";
...
import { DEFAULT_TRANSACTION_MASTER_DATA, type TransactionMasterFormData } from "@/components/TransactionMaster/AddTransactionMaster";
...
/* ===== from TlDisbursementAuthorizeTable.tsx ===== */
export type TlDisbursementAuthorizeTable_TlDisbursementAuthorizeTab = "new" | "rejected";

export type TlDisbursementAuthorizeTable_TlDisbursementAuthorizeRow = {
  srNo: number;
  scrollNo: string;
  accountCode: string;
  amount: string;
  ...
  createdBy: string;
  createdDate: string;
  tab: TlDisbursementAuthorizeTable_TlDisbursementAuthorizeTab;
};

const TlDisbursementAuthorizeTable_columns = [
...
const TlDisbursementAuthorizeTable_SAMPLE_DISBURSEMENTS: Omit<TlDisbursementAuthorizeTable_TlDisbursementAuthorizeRow, "srNo" | "tab">[] = [
  ...
  { scrollNo: "123", accountCode: "00022010000006", amount: "300.0", userId: "ABC", createdBy: "Admin", createdDate: "28-May-2026" },
  { scrollNo: "124", accountCode: "00022010000007", amount: "450.0", userId: "ABC", createdBy: "Admin", createdDate: "29-May-2026" },
  { scrollNo: "125", accountCode: "00022010000008", amount: "600.0", userId: "ABC", createdBy: "Admin", createdDate: "30-May-2026" },
  { scrollNo: "126", accountCode: "00022010000009", amount: "820.0", userId: "ABC", createdBy: "Admin", createdDate: "31-May-2026" },
  { scrollNo: "127", accountCode: "00022010000010", amount: "150.0", userId: "ABC", createdBy: "Admin", createdDate: "01-Jun-2026" },
  { scrollNo: "128", accountCode: "00022010000011", amount: "980.0", userId: "ABC", createdBy: "Admin", createdDate: "02-Jun-2026" },
  { scrollNo: "129", accountCode: "00022010000012", amount: "375.0", userId: "ABC", createdBy: "Admin", createdDate: "03-Jun-2026" },
  { scrollNo: "130", accountCode: "00022010000013", amount: "525.0", userId: "ABC", createdBy: "Admin", createdDate: "04-Jun-2026" },
  { scrollNo: "131", accountCode: "00022010000014", amount: "690.0", userId: "ABC", createdBy: "Admin", createdDate: "05-Jun-2026" },
  { scrollNo: "132", accountCode: "00022010000015", amount: "410.0", userId: "ABC", createdBy: "Admin", createdDate: "06-Jun-2026" },
  { scrollNo: "133", accountCode: "00022010000016", amount: "860.0", userId: "ABC", createdBy: "Admin", createdDate: "07-Jun-2026" },
  { scrollNo: "134", accountCode: "00022010000017", amount: "220.0", userId: "ABC", createdBy: "Admin", createdDate: "08-Jun-2026" },
  { scrollNo: "135", accountCode: "00022010000018", amount: "710.0", userId: "ABC", createdBy: "Admin", createdDate: "09-Jun-2026" },
  { scrollNo: "136", accountCode: "00022010000019", amount: "340.0", userId: "ABC", createdBy: "Admin", createdDate: "10-Jun-2026" },
  { scrollNo: "137", accountCode: "00022010000020", amount: "925.0", userId: "ABC", createdBy: "Admin", createdDate: "11-Jun-2026" },
];

const TlDisbursementAuthorizeTable_buildRows = (tab: TlDisbursementAuthorizeTable_TlDisbursementAuthorizeTab, count: number): TlDisbursementAuthorizeTable_TlDisbursementAuthorizeRow[] =>
  Array.from({ length: count }, (_, i) => ({
    ...TlDisbursementAuthorizeTable_SAMPLE_DISBURSEMENTS[i % TlDisbursementAuthorizeTable_SAMPLE_DISBURSEMENTS.length],
    scrollNo: String(118 + i + (tab === "rejected" ? 100 : 0)),
    srNo: i + 1,
    tab,
  }));

export const TlDisbursementAuthorizeTable_TL_DISBURSEMENT_TAB_COUNTS: Record<TlDisbursementAuthorizeTable_TlDisbursementAuthorizeTab, number> = {
  new: 20,
  rejected: 18,
};

const TlDisbursementAuthorizeTable_ALL_ROWS: TlDisbursementAuthorizeTable_TlDisbursementAuthorizeRow[] = [
  ...TlDisbursementAuthorizeTable_buildRows("new", TlDisbursementAuthorizeTable_TL_DISBURSEMENT_TAB_COUNTS.new),
  ...TlDisbursementAuthorizeTable_buildRows("rejected", TlDisbursementAuthorizeTable_TL_DISBURSEMENT_TAB_COUNTS.rejected),
];

const TlDisbursementAuthorizeTable_PAGE_SIZE = 15;

type TlDisbursementAuthorizeTable_SortKey = Exclude<(typeof TlDisbursementAuthorizeTable_columns)[number]["key"], "action">;

type TlDisbursementAuthorizeTable_TlDisbursementAuthorizeTableProps = {
  activeTab: TlDisbursementAuthorizeTable_TlDisbursementAuthorizeTab;
  filters?: TlDisbursementFilterModal_TlDisbursementFilters;
  onAuthorize?: (row: TlDisbursementAuthorizeTable_TlDisbursementAuthorizeRow) => void;
};

const TlDisbursementAuthorizeTable = ({ activeTab, filters, onAuthorize }: TlDisbursementAuthorizeTable_TlDisbursementAuthorizeTableProps) => {
  const [sortKey, setSortKey] = useState<TlDisbursementAuthorizeTable_SortKey | null>(null);
  const [sortAsc, setSortAsc] = useState(true);
  const [page, setPage] = useState(1);

  const handleSort = (key: TlDisbursementAuthorizeTable_SortKey) => {
    if (sortKey === key) {
      setSortAsc(!sortAsc);
    } else {
      setSortKey(key);
  ...
  const filteredRows = TlDisbursementAuthorizeTable_ALL_ROWS.filter((r) => {
  ...
  const totalPages = Math.max(1, Math.ceil(sortedRows.length / TlDisbursementAuthorizeTable_PAGE_SIZE));
  const currentPage = Math.min(page, totalPages);
  const pageRows = sortedRows.slice((currentPage - 1) * TlDisbursementAuthorizeTable_PAGE_SIZE, currentPage * TlDisbursementAuthorizeTable_PAGE_SIZE);
        ...
        <table className="w-full min-w-[1100px] table-fixed border-collapse">
          <thead>
            <tr className="bg-primary">
              {TlDisbursementAuthorizeTable_columns.map((col) => (
                <th
                  key={col.key}
                  onClick={() => col.sortable && handleSort(col.key as TlDisbursementAuthorizeTable_SortKey)}
          ...
          <tbody>
            {pageRows.length === 0 ? (
              <tr>
                <td colSpan={TlDisbursementAuthorizeTable_columns.length} className="px-6 py-10 text-center text-sm text-gray-400 dark:text-slate-500">
                  No records found
                </td>
              </tr>
            ) : (
...
/* ===== from TlDisbursementFilterModal.tsx ===== */
const TlDisbursementFilterModal_filterOptions = [
  {
    id: "accountCode",
    label: "Account Code",
...
type TlDisbursementFilterModal_FilterKey = (typeof TlDisbursementFilterModal_filterOptions)[number]["id"];

export type TlDisbursementFilterModal_TlDisbursementFilters = Record<TlDisbursementFilterModal_FilterKey, string>;

type TlDisbursementFilterModal_TlDisbursementFilterModalProps = {
  onClose: () => void;
  onApply: (filters: TlDisbursementFilterModal_TlDisbursementFilters) => void;
  initialValues?: TlDisbursementFilterModal_TlDisbursementFilters;
};

export const TlDisbursementFilterModal_defaultTlDisbursementFilters: TlDisbursementFilterModal_TlDisbursementFilters = {
  accountCode: "",
  scrollNo: "",
  amount: "",
  userId: "",
...
function TlDisbursementFilterModal({
  onClose,
  onApply,
  initialValues = TlDisbursementFilterModal_defaultTlDisbursementFilters,
}: TlDisbursementFilterModal_TlDisbursementFilterModalProps) {
  const [activeFilter, setActiveFilter] = useState<TlDisbursementFilterModal_FilterKey>("accountCode");
  const [values, setValues] = useState<TlDisbursementFilterModal_TlDisbursementFilters>(initialValues);

  const active = TlDisbursementFilterModal_filterOptions.find((f) => f.id === activeFilter);
  ...
  const handleClearAll = () => {
    setValues(TlDisbursementFilterModal_defaultTlDisbursementFilters);
    onApply(TlDisbursementFilterModal_defaultTlDisbursementFilters);
    onClose();
  };
    ...
    <div className="relative w-full max-w-4xl overflow-hidden rounded-3xl border-2 border-primary bg-white p-8 dark:bg-slate-900">
      ...
      <div className="relative z-10 mt-8 flex items-start gap-0">
        <div className="flex w-full max-w-[470px] flex-col gap-4">
          {TlDisbursementFilterModal_filterOptions.map((option) => {
...
/* ===== from AuthorizeTlDisbursementModal.tsx ===== */
export interface AuthorizeTlDisbursementModal_AuthorizeTlDisbursementModalProps {
  open: boolean;
...
const AuthorizeTlDisbursementModal_SectionIcon = () => (
  <div className="flex h-10 w-10 items-center justify-center rounded-full bg-primary-100">
    <User size={20} className="text-primary" />
  </div>
);

const AuthorizeTlDisbursementModal_DisabledLookupTrigger = () => (
  <button
    type="button"
    disabled
    aria-hidden
...
const AuthorizeTlDisbursementModal_AuthorizeTlDisbursementFooter = ({
  onReject,
  onCancel,
  onAuthorize,
}: {
...
const AuthorizeTlDisbursementModal = ({
  ...
  onAuthorize,
  onReject,
}: AuthorizeTlDisbursementModal_AuthorizeTlDisbursementModalProps) => {
      ...
      <FormModal
        ...
        headerIcon={
          <div className="flex h-12 w-12 items-center justify-center">
            <Image src={IMAGES.AUTHORIZE_USER} alt="Authorize TL Disbursement" width={50} height={50} />
          </div>
        }
        tabs={[]}
        activeTab=""
        ...
        maxWidth="max-w-7/8"
        customFooter={
          <AuthorizeTlDisbursementModal_AuthorizeTlDisbursementFooter onReject={handleReject} onCancel={onClose} onAuthorize={handleAuthorize} />
        }
      >
        <SectionCard
          ...
          icon={<AuthorizeTlDisbursementModal_SectionIcon />}
        >
          <div className={`${grid4} mt-2`}>
            ...
            <FieldShell label="Account Code" labelHi="खाते कोड" required>
              <div className="flex items-center gap-2">
                ...
                <AuthorizeTlDisbursementModal_DisabledLookupTrigger />
              </div>
            </FieldShell>
...
/* ===== from TlDisbursementAuthorizePage.tsx ===== */
const TABS = [
  { key: "new" as const, labelKey: "authorizeAccount.tabs.newAuthorization", count: TlDisbursementAuthorizeTable_TL_DISBURSEMENT_TAB_COUNTS.new },
  { key: "rejected" as const, labelKey: "authorizeAccount.tabs.authorizeRejected", count: TlDisbursementAuthorizeTable_TL_DISBURSEMENT_TAB_COUNTS.rejected },
];

const TlDisbursementAuthorizePage = () => {
  const router = useRouter();
  const [activeTab, setActiveTab] = useState<AuthorizationTabKey>("new");
  const [isFilterOpen, setIsFilterOpen] = useState(false);
  const [isSearchVisible, setIsSearchVisible] = useState(false);
  const [filters, setFilters] = useState<TlDisbursementFilterModal_TlDisbursementFilters>(TlDisbursementFilterModal_defaultTlDisbursementFilters);
  const [authorizeRow, setAuthorizeRow] = useState<TlDisbursementAuthorizeTable_TlDisbursementAuthorizeRow | null>(null);

  const handleAuthorize = (row: TlDisbursementAuthorizeTable_TlDisbursementAuthorizeRow) => setAuthorizeRow(row);
    ...
    <div className="min-h-screen app-page-bg no-scrollbar dark:bg-slate-950">
      <GlobalNav
        ...
        breadcrumbs={[
          ...
          { label: "TL Disbursement Authorize", href: "#" },
        ]}
        onBack={() => router.back()}
      />
...
export default TlDisbursementAuthorizePage;
