...
export default function BranchMasterPage() {
  ...
  const navConfig = useMemo(() => {
    ...
    const currentRole = roleConfig[role as keyof typeof roleConfig] || roleConfig.clerk;
        ...
        <div className="flex items-center gap-6 border-b border-gray-200 px-2 overflow-x-auto no-scrollbar">
          {tabs.map((tab) => (
            ...
            >
              {tab}
              {activeTab === tab && (
                <span className="absolute bottom-[-1px] left-0 w-full border-b-3 bg-blue-600 rounded-t-sm" />
              )}
            </button>
          ))}
        </div>
