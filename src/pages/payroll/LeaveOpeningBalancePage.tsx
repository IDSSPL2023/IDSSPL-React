...
import LeaveOpeningBalanceFormModal from "@/pages/payroll/Master/LeaveOpeningBalanceFormModal";
...
const LeaveOpeningBalancePage = () => {
  const router = useRouter();
  const [filters, setFilters] = useState<CustomerFilters>(defaultValues);
  const [isSearchVisible, setIsSearchVisible] = useState(false);
  const [isFilterOpen, setIsFilterOpen] = useState(false);
  const [balanceEmployee, setBalanceEmployee] = useState<PayrollRow | null>(null);
    ...
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-slate-950">
      ...
      <main className="p-4 sm:p-6">
        <PayrollTable
          rows={filteredRows}
          actions={(row) => [
            { key: "leave-opening-balance", label: "Leave Opening Balance", icon: CalendarClock, onClick: () => setBalanceEmployee(row) },
          ]}
        />
      </main>
...
export default LeaveOpeningBalancePage;
